...
def get_all(skip: int = 0, limit: int = 100):
    ...
    conn.db.open()
    conn.db.cursor.execute("SELECT * FROM persona")
    res = conn.db.cursor.fetchall()
    conn.db.close()
    return res
...
def find_by_codigo(codigo: str):
    conn.db.open()
    query = f"SELECT idpersona, idusuario, nombres, apellidos, direccion, telefono, tipo, estado, fecha, codigo FROM persona WHERE codigo = '{codigo}'"
